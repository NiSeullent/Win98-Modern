"""Static Win98 PE/import gate for the bounded FindFirstStreamW port."""
...
def check_exe(path: Path, native: set[str], static_import: bool) -> list[str]:
    ...
    if set(imported) != {"KERNEL32.DLL"}:
        ...
        if static_import and "FindFirstStreamW" not in names:
            issues.append("FindFirstStreamW is not a static KERNEL32 import")
        if not static_import and "FindFirstStreamW" in names:
            issues.append("direct test unexpectedly imports FindFirstStreamW")
        permitted = native | ({"FindFirstStreamW"} if static_import else set())
...
def main() -> int:
    base = Path("build/stream")
    try:
        issues = check(
            base / "m98wrap.dll",
            base / "stream_smoke.exe",
            base / "stream_import_probe.exe",
            Path("benchmarks/win98se-ko-oem-native-exports-v1.json"),
        )
    except (OSError, KeyError, ValueError, pefile.PEFormatError) as exc:
        print(f"FAIL: FindFirstStreamW PE gate: {exc}", file=sys.stderr)
        return 1
    if issues:
        for issue in issues:
            print(f"FAIL: {issue}", file=sys.stderr)
        return 1
    print("PASS: FindFirstStreamW Win98 PE/import static gate")
    return 0
