"""Static PE/import gate for QueryFullProcessImageNameA/W."""
...
PORT_NAMES = {"QueryFullProcessImageNameA", "QueryFullProcessImageNameW"}
...
def check_exe(path: Path, native: set[str], static_import: bool) -> list[str]:
    ...
    if set(imported) != {"KERNEL32.DLL"}:
        ...
        actual = names & PORT_NAMES
        expected = PORT_NAMES if static_import else set()
        if actual != expected:
            issues.append(
                f"{path.name} process-path imports {sorted(actual)}, expected {sorted(expected)}"
            )
        permitted = native | (PORT_NAMES if static_import else set())
...
def main() -> int:
    base = Path("build/processpath")
    try:
        issues = check(
            base / "m98wrap.dll",
            base / "processpath_smoke.exe",
            base / "processpath_import_probe.exe",
            Path("benchmarks/win98se-ko-oem-native-exports-v1.json"),
        )
    except (OSError, KeyError, ValueError, pefile.PEFormatError) as exc:
        print(f"FAIL: process-path PE gate: {exc}", file=sys.stderr)
        return 1
    if issues:
        for issue in issues:
            print(f"FAIL: {issue}", file=sys.stderr)
        return 1
    print("PASS: process-path Win98 PE/import static gate")
    return 0
