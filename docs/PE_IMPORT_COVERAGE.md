...
전체 표면의 재현 가능한 측정에는 [대상 앱과 SDK 범위](TARGET_APPS.md)에 적은 고정 Windows SDK `10.0.28000.2705`의 API를 범주별로 분류한 별도 인벤토리가 필요하다. Win32 DLL 함수뿐 아니라 COM 인터페이스, WinRT, 시스템 서비스 등 포함 여부와, A/W 변형·서수·별칭·버전별 API를 각각 어떻게 셀지 정해야 한다. 헤더의 선언 수나 PE 내보내기 수를 그대로 전체 API 수로 쓰면 유형 정의·매크로·비공개 내보내기와 실제 호출 가능 API가 혼재한다. 각 항목의 의미·동작 시험 결과까지 갖춘 뒤 전체 분모와 구현된 분자를 비교해야 한다. **현재 이 프로젝트에는 그 SDK 기준 인벤토리와 전체 표면 측정치가 없다.**
