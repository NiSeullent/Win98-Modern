# Isolated host build/test for the GetDateFormatEx addition. Guest tests are separate.
param([switch]$SkipHostExecution)
$ErrorActionPreference = 'Stop'
$projectRoot = Split-Path -Parent $PSScriptRoot
$outDir = Join-Path $projectRoot 'build/dateformat'
...
& $compiler @testFlags '-o' (Join-Path $outDir 'dateformat_smoke.exe') (Join-Path $projectRoot 'tests/dateformat_smoke.c') '-lkernel32'
if ($LASTEXITCODE -ne 0) { throw 'dateformat_smoke.exe build failed' }
& $compiler @testFlags '-o' (Join-Path $outDir 'dateformat_import_probe.exe') (Join-Path $projectRoot 'tests/dateformat_import_probe.c') '-lkernel32'
if ($LASTEXITCODE -ne 0) { throw 'dateformat_import_probe.exe build failed' }
...
try {
    & python 'tests/check_dateformat_pe98.py'
    if ($LASTEXITCODE -ne 0) { throw 'GetDateFormatEx static PE gate failed' }
    if (-not $SkipHostExecution) {
        & (Join-Path $outDir 'dateformat_smoke.exe')
        if ($LASTEXITCODE -ne 0) { throw 'GetDateFormatEx direct host smoke failed' }
        & (Join-Path $outDir 'dateformat_import_probe.exe')
        if ($LASTEXITCODE -ne 0) { throw 'GetDateFormatEx import host smoke failed' }
