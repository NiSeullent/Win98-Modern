...
$exeFlags = @('-std=c11','-Os','-Wall','-Wextra','-Werror',
    ...
    '-Wl,--minor-image-version,10','-Wl,--disable-dynamicbase',
    '-Wl,--disable-nxcompat','-Wl,--disable-tsaware')
& $cc @dllFlags '-o' (Join-Path $out 'm98wrap.dll') (Join-Path $root 'src/m98wrap.c') (Join-Path $root 'src/m98nls_ex.c') (Join-Path $root 'src/m98_threadpool.c') (Join-Path $root 'src/m98_initonce.c') (Join-Path $root 'src/wine_uppercase.c') '-lkernel32'
