...
Notepad++ 8.9.8 x86 포터블 앱의 단계별 실행 시험에서는 `DBGHELP.DLL` 누락, 이어 `DWMAPI.DLL` 누락, 두 앱 로컬 브리지를 제공한 뒤 실행 파일 포맷 오류가 순서대로 관찰됐습니다. PE 헤더 버전 6.0을 로컬 시험 복사본에서만 4.10으로 바꿔도 포맷 오류가 계속됐습니다. 앱이 실행됐다는 증거는 없으며, 시험용 앱 파일과 ISO는 Git에서 제외합니다. 상세 내용은 `docs/TARGET_APPS.md`에 있습니다.
